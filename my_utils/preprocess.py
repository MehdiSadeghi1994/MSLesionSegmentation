...
def preprocess_img(inputfile, output_preprocessed, zooms):
    ...
    corrected_image = sitk.GetArrayFromImage(output)[:, :, :, None].squeeze()
    print('   Bias Correction Completed...')

    data_sub = corrected_image - gaussian_filter(corrected_image, sigma=2)
    if 'FLAIR' in output_preprocessed:
        clahe = cv2.createCLAHE(clipLimit =2.0, tileGridSize=(8,8))
        temp = corrected_image.astype(np.uint16)
        data_clahe = np.zeros_like(data)
        for s in range(temp.shape[2]):
            data_clahe[:,:,s] = clahe.apply(temp[:,:,s].squeeze())
        print('   FLAIR Histogram Equalization Completed...')
    else:
        data_clahe = corrected_image
        print('   T1 Histogram Equalization Completed...')
    ...
    data_clahe = gaussian_filter(data_clahe, sigma=2)
    # data = np.concatenate((data_clahe, corrected_image), 3)
    data = np.concatenate((np.expand_dims(data_clahe,3), np.expand_dims(data_sub,3)), 3)
